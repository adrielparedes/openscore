...
public abstract class Repository<T extends Storable> {
    ...
    public List<T> findAllWithoutDeleted(Page page) {
        TypedQuery<T> query = this.createQuery(MessageFormat.format("from {0} WHERE deleted = false ORDER BY id ASC",
                this.persistentClass.getSimpleName()));
        return this.findByQuery(query,
                page);
    }

    public List<T> findAllWithoutDeleted() {
        TypedQuery<T> query = this.createQuery(MessageFormat.format("from {0} WHERE deleted = false ORDER BY id ASC",
                this.persistentClass.getSimpleName()));
        return this.findByQuery(query);
    }
    ...
    public void enableById(long id) {
        Optional<T> found = this.findByIdWithDeleted(id);
        found.ifPresent(elem -> {
